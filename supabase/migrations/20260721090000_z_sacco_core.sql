...
declare s sessions%rowtype; mid uuid; mn text; docs jsonb:=coalesce(payload->'kycDocuments','[]'::jsonb);
begin
 ...
 if coalesce(payload->>'memberId','')<>'' then
   mid:=(payload->>'memberId')::uuid;
   if not exists(select 1 from members where id=mid and sacco_id=s.sacco_id) then raise exception 'Member not found.'; end if;
   if exists(select 1 from members where sacco_id=s.sacco_id and id<>mid and lower(email)=lower(payload->>'email')) then raise exception 'Member email is already registered.'; end if;
   update members set name=payload->>'name',phone=payload->>'phone',email=payload->>'email',branch=payload->>'branch',national_id=payload->>'nationalId',member_type=payload->>'memberType',address=payload->>'address',password_hash=case when coalesce(payload->>'passwordHash','')<>'' then payload->>'passwordHash' else password_hash end,documents=case when jsonb_array_length(docs)>0 then docs else documents end,profile_photo=coalesce((select d->>'dataUrl' from jsonb_array_elements(docs) d where coalesce((d->>'isProfilePhoto')::boolean,false) limit 1),profile_photo) where id=mid and sacco_id=s.sacco_id;
